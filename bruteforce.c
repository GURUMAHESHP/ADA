...
int brute_force(char text[], char pattern[])
{
    ...
    for (int i = 0; i <= N - M; i++) {
        ...
        step_count++; // for the mismatch/last check
        if (j == M) {
...
void run_analysis()
{
    ...
    for (int n = 10; n <= 200; n += 10) {
        ...
        // ===== Best Case =====
        // Text of 'a's, pattern = "b" (mismatch at first character always)
        ...
        // ===== Worst Case =====
        // Text of all 'a's, pattern = "aaa...a" (length = n/2 for more cost)
        for (int i = 0; i < n; i++) text[i] = 'a';
        text[n] = '\0';
        for (int i = 0; i < n / 2; i++) pattern[i] = 'a';
        pattern[n/2] = '\0';
        brute_force(text, pattern);
        worst = step_count;
